...
void coderAnsh() {
    ...
    ll xa = cord[a-1].first;
    ll ya = cord[a-1].second;
    ll xb = cord[b-1].first;
    ll yb = cord[b-1].second;
    ...
    f(i, 0, k) {
        ll curr_a = abs(cord[i].first - xa) + abs(cord[i].second - ya);
        ll curr_b = abs(cord[i].first - xb) + abs(cord[i].second - yb);

        mina = min(mina, curr_a);
        minb = min(minb, curr_b);
    }
