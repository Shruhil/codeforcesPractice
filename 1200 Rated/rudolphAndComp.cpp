...
class cmp {
public:
    bool operator()(pair<int,pair<ll,int>> &a, pair<int,pair<ll,int>> &b) {
...
void coderAnsh() {
    ...
    vector<pair<int,pair<ll, int>>> queTime(n);
    f(i, 0, n) {
        int j = 0;
        ll pen = 0;
        ll time = 0;
        ...
        queTime[i] = make_pair(q, make_pair(pen, i));
    }

    sort(queTime.begin(), queTime.end(), cmp());

    f(i, 0, n) {
        if(queTime[i].second.second == 0) {
            cout << i + 1;
            return;
        }
    }
