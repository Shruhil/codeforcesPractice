...
void coderAnsh() {
    ...
    int x = find_if(arr.begin(), arr.end(), [&](int v) { return v > arr[k]; }) - arr.begin();

    vi pos(n);
    iota(pos.begin(), pos.end(), 0); // position vector
    ...
    for (int i : {0, x}) {
        ...
        vi stt(n, 0); // stt[i] = number of wins of i
