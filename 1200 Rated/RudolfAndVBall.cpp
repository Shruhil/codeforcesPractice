...
void dfs(int ind, int x, int n, int m, set<int> &players, vector<vector<bool>> &vis, vector<pair<int,char>> &moves) {
	if(ind == m) {
		players.insert(x);
		return;
	}
	...
	vis[ind][x]= true;
	char movement = moves[ind].second;
	int steps = moves[ind].first;

	if(movement == '0') {
		dfs(ind+1, (x + steps)%n == 0 ? n : (x + steps)%n, n, m, players, vis, moves);
	}

	if(movement == '1') {
		dfs(ind+1, (x + (n - steps))%n == 0 ? n :  (x + (n - steps))%n, n, m, players, vis, moves);
	}
