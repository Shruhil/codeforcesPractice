...
void coderAnsh() {
    ...
    unordered_map<int,bool> isPresent1, isPresent2;
    vector<int> preMex(n), suffMex(n);
    int mex1 = 0, mex2 = 0;

    f(i, 0, n) {
        if(mex1 == arr[i]) {
            mex1+=1;
            while(isPresent1[mex1]) {
                mex1+=1;
            }
        }
        
        isPresent1[arr[i]] = true;
        preMex[i] = mex1;
    }
    ...
    rf(i, n, 0) {
        if(mex2 == arr[i]) {
            mex2+=1;
            while(isPresent2[mex2]) {
                mex2+=1;
            }
        }
        
        isPresent2[arr[i]] = true;
        suffMex[i] = mex2;
    }

    f(i, 0, n-1) {
        if(preMex[i] == suffMex[i+1]) {
            cout << 2 << '\n';
            cout << 1 << " " << i + 1 << '\n';
            cout << i + 2 << " " << n;
            return;
