...
// input output 
void input(vi &arr) {for(auto &x : arr) cin >> x;}
void print(vi &arr) {for(auto &x : arr) cout << x;}
...
void coderAnsh() {
	...
	int maxEle = 2*n;
	vi freqa(maxEle + 1), freqb(maxEle + 1);
	...
	while(i < n) {
		...
		freqb[ele] = max(freqb[ele], freq);
		i = j;
	}

	int maxFreq = 0;
	f(i, 1, maxEle + 1) {
		maxFreq = max(maxFreq, freqa[i] + freqb[i]);
	}
